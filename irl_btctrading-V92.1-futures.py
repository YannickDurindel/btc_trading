...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 12 10:23:11 2022
...
while True :
    try :
        ...
        if len(btc_pastprice)> 13:
            slope = []
            for i in range(1,13):
                slope.append(btc_pastprice[-i]/btc_pastprice[-i-1])
            trend = (sum(slope[0:12])/12-1)*10000
            
        if btc_newprice > btc_prevprice*1.005 and  btc_wallet == False and trend < -1*buysell_lim :
        ...
        elif btc_newprice*1.005 < btc_max and btc_wallet == True and trend > buysell_lim:
