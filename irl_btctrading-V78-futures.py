...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 25 19:10:54 2022
...
while True :
    try:
        ...
        if btc_newprice > btc_prevprice*1.0015 and W == 0:
            ...
            time.sleep(100)
            btc_newprice = float(binance_client.futures_symbol_ticker(symbol='BTCUSDT')['price'])
            if btc_newprice*1.0025 < btc_prevprice:
                time.sleep(200)
                btc_newprice = float(binance_client.futures_symbol_ticker(symbol='BTCUSDT')['price'])
                if btc_newprice*1.004<btc_prevprice:
                    time.sleep(200)
                    btc_newprice = float(binance_client.futures_symbol_ticker(symbol='BTCUSDT')['price'])
                    if btc_newprice*1.006<btc_prevprice:
                        time.sleep(300)
                        btc_newprice = float(binance_client.futures_symbol_ticker(symbol='BTCUSDT')['price'])
            ...
            btc_prevprice = btc_newprice
        elif btc_newprice*1.02 < btc_prevprice and W == 0:
            W = P*btc_newprice*20
            P = 0
            print("sell, wallet =",W)
        time.sleep(1)
