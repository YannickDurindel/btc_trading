...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  7 17:12:11 2022
...
while True :
    try :
        ...
        elif len(btc_pastprice)<151 and len(btc_pastprice)>61 :
            slope = []
            for i in range(1,61):
                slope.append(btc_pastprice[-i]/btc_pastprice[-i-1])
            trend_middle = (sum(slope[0:60])/60-1)*10000
            trend_short = (sum(slope[48:60])/12-1)*10000
        elif len(btc_pastprice)<151 and len(btc_pastprice)>12 :
            slope = []
            for i in range(1,13):
                slope.append(btc_pastprice[-i]/btc_pastprice[-i-1])
            trend_short = (sum(slope[0:12])/12-1)*10000
            
        if btc_newprice > btc_prevprice*1.005 and  btc_wallet == False and trend_short < 3 and trend_middle < 0.6 and trend_long < 0.1 :
        ...
        elif btc_newprice*1.005 < btc_max and btc_wallet == True and trend_short > -3 and trend_middle > -0.6 and trend_long > -0.1 :
        ...
        elif btc_newprice*1.001 > btc_prevprice and btc_wallet == False and (trend_short < -3 or trend_middle < -0.6) :
            #binance_client.futures_create_order(symbol='BTCUSDT',type='MARKET',side='SELL',quantity=btc_buysell)
            print("sell for safety")
            message += "sell,for safety, wallet = "+str(getwallet("USDT"))+", the low gap was "+str(btc_prevprice/btc_min)+", btc price = "+str(btc_newprice)+"\n"
            btc_prevprice = btc_newprice
            btc_max = btc_newprice
            btc_wallet = True
            btc_buysell = round((wallet-1)*leverage/btc_newprice,3)
        ...
        if btc_wallet == False:
            while btc_newprice > btc_prevprice and trend_short < 3 and trend_middle < 0.6 and trend_long < 0.1 : 
                #binance_client.futures_create_order(symbol='BTCUSDT',type='MARKET',side='SELL',quantity=btc_buysell)
                print("sell")
                message += "sell, wallet = "+str(getwallet("USDT"))+", the low gap was "+str(btc_prevprice/btc_min)+", btc price = "+str(btc_newprice)+"\n"
                btc_prevprice = btc_newprice
