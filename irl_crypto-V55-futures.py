...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar  4 21:38:53 2022
...
def btc_timer(deltaT,btc_crisis_counter,btc_crisis,btc_newprice,btc_prevprice,btc_wallet):
    if btc_crisis == False:
        t1 = time.time()
        ...
        btc_crisis_counter = False
        if deltaT != 0:
            print("lowering the price of the btc for",deltaT,'sec')
    if btc_crisis == True and time.time()-t1>deltaT:
        btc_crisis = False
        btc_crisis_counter = True
        btc_wallet = False
    return btc_crisis_counter,btc_wallet
...
while True :
    try :
        ...
        if btc_newprice > btc_prevprice*1.001 and btc_wallet == False:
            ...
            btc_crisis_counter = btc_timer(30,btc_crisis_counter,btc_crisis,btc_newprice,btc_prevprice,btc_wallet)[0]
            btc_wallet = btc_timer(0,btc_crisis_counter,btc_crisis,btc_newprice,btc_prevprice,btc_wallet)[1]
            btc_newprice = float(binance_client.futures_symbol_ticker(symbol='BTCUSDT')['price'])
