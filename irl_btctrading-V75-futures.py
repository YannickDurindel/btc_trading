...
# -*- coding: utf-8 -*-
"""
Created on Fri Mar 25 18:30:37 2022
...
while True :
    try :
        ...
        if W > Wi*2:
            ...
            message = "you just doubled your networh, you earned"+str(W-Wi)+", your wallet ="+str(W)
        ...
        if btc_newprice > btc_prevprice*1.0015 and btc_wallet == False:
            ...
            btc_prevprice = btc_newprice
            btc_wallet = True
            print("sell btc, btc_price =",btc_newprice,"wallet ",W,'lowering to',btc_newprice*1.0015,'\n')
            ...
            W = P*btc_newprice
            btc_prevprice = btc_newprice
            print("sell btc, btc_price =",btc_newprice,"wallet =",W,'\n')    
            btc_wallet = True
            with open('performances.csv', 'a', newline='') as P:  
                Perf = writer(P)
                Perf.writerow([(time.time()-t_init)+t0,wallet,btc_newprice])  
        ...
        tf = time.time()
        print("it took",tf-t0,"to raise",W-Wii,"%")
        with open('performances.csv', 'a', newline='') as P:  
            Perf = writer(P)
            Perf.writerow([(time.time()-t_init)+t0,wallet,btc_newprice])  
            P.close()
