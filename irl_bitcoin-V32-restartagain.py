...
# -*- coding: utf-8 -*-
"""
Created on Sat Feb 26 10:05:05 2022
...
while wallet <= 30 :
    ...
    if btc_newprice*1.00015 < btc_prevprice and wallet > portfolio*btc_newprice :
    ...
    elif btc_newprice > btc_prevprice*1.0001 and wallet < portfolio*btc_newprice :
        fees += 0.075
        ...
        n = 0
        print("hell case ! sorry, you're gonna have to wait :(")
        while btc_newprice < btc_prevprice :
            btc_newprice = float(client.get_symbol_ticker(symbol="BTCUSDT")['price'])
            n+=1
            if n > 500 :
                if btc_newprice*1.003 > btc_newprice :
                    print('sell, interest =',(wallet*100/walletinit)-100,' total fees =',fees,' final gain = ',(wallet*100/walletinit)-100-fees,'\n')
                    client.create_order(symbol="BTCUSDT",side='sell',type='MARKET',quantity=round(portfolio*0.99,5))
                    print('sold to finally exit the hellcase, loss = ',float(client.get_asset_balance(asset='USDT')['free'])*btc_newprice/btc_prevprice)
                    hellcase_loss += float(client.get_asset_balance(asset='USDT')['free'])*btc_newprice/btc_prevprice
            time.sleep(0.5)
        btc_prevprice = btc_newprice
